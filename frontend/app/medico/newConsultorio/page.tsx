...
export default function NewConsultorioPage() {
  ...
  useEffect(() => {
    if (!loading && (!user || !user.is_medico)) router.push('/login');
      ...
      if (user?.is_medico) {
        try {
          ...
          const response = await axios.get(
            'https://sanarebackend-production.up.railway.app/api/clinicas/',
            { headers: { Authorization: `Bearer ${accessToken}` } }
          );
          const misClinicas = response.data.filter(
            (c: any) => c.medico_responsable?.id === user.id
          );
          if (misClinicas.length > 0) setClinicaExistente(true);
        } catch (err) {
          // console.error(err);
        }
      }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = () => reader.result && setImagenPreview(reader.result as string);
      reader.readAsDataURL(file);
    }
  };

  const handleDiaChange = (diaId: number) => {
    setDiasHabiles(prev => (prev.includes(diaId) ? prev.filter(d => d !== diaId) : [...prev, diaId]));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nombre || !formData.descripcion || !formData.ubicacion || !formData.hora_apertura || !formData.hora_cierre || diasHabiles.length === 0) {
      setError('Rellena todos los campos y selecciona al menos un día hábil.');
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        if (error.response.status === 401) {
          ...
          router.push('/login');
        } else setError(`Error del servidor: ${JSON.stringify(error.response.data)}`);
    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="px-4 py-10 md:px-8 lg:px-16">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl md:text-3xl font-bold mb-6 text-center">Agregar Nuevo Consultorio</h1>
              ...
              <div className="flex flex-col md:flex-row gap-8 mt-6">
                ...
                <div className="relative w-full md:w-1/2 flex justify-center items-center">
                  <Image
                    ...
                    width={600}
                    height={400}
                    className="rounded-lg shadow object-cover w-full h-auto max-h-96"
                  />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {DIAS_SEMANA.map(d => (
                        <label key={d.id} className="flex items-center gap-2">
                          <input type="checkbox" checked={diasHabiles.includes(d.id)} onChange={() => handleDiaChange(d.id)} />
                          {d.nombre}
                        </label>
                      ))}
                    </div>
